...
export default function LailaPinkLogin() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const users = [
        {
          email: "adminlailaps@gmail.com",
          password: "123",
          role: "SUPER_ADMIN",
          redirect: "/super-admin/dashboard",
        },
        {
          email: "adminbatu@gmail.com",
          password: "123",
          role: "ADMIN",
      ...
      // Proteksi spasi dan case-sensitive huruf kapital
      ...
      // 2. Simpan ke Cookies (Penting untuk deteksi Middleware & SSR Guard)
      document.cookie =
        "token=dummy-token; path=/; max-age=86400; SameSite=Lax";
      document.cookie = `user=${encodeURIComponent(
        JSON.stringify({ email: user.email, role: user.role }),
      )}; path=/; max-age=86400; SameSite=Lax`;

      console.log("Login sukses! Mengarahkan ke:", user.redirect);

      // Menggunakan replace langsung dari window browser agar tidak tersangkut cache router
    ...
    <div className="min-h-screen bg-[#FFF5F7] text-[#4A1D24] font-sans flex items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="w-full max-w-[450px] relative z-10 animate-in fade-in zoom-in duration-700">
        ...
        <div className="bg-white/70 backdrop-blur-2xl border border-white rounded-[3.5rem] p-10 shadow-[0_20px_50px_rgba(244,114,182,0.1)]">
          ...
          {/* Dummy Account Info (Struktur Tag Div Diperbaiki) */}
          <div className="mt-8 bg-pink-50 rounded-2xl p-5 border border-pink-100 text-xs space-y-3">
            <div className="font-black uppercase tracking-widest text-pink-500">
              Dummy Account
            </div>
            <div className="border-b border-pink-100/50 pb-2">
              <div className="font-bold text-pink-950">SUPER ADMIN</div>
              <div className="text-pink-800/70">adminlailaps@gmail.com</div>
              <div className="text-pink-400 font-medium">Password: 123</div>
            </div>
            <div>
              <div className="font-bold text-pink-950">ADMIN OUTLET</div>
              <div className="text-pink-800/70">adminbatu@gmail.com</div>
              <div className="text-pink-400 font-medium">Password: 123</div>
            </div>
          </div>
        </div>
